...
**Architecture:** 采用 `Next.js + Prisma + Postgres + Redis + BullMQ + 本地文件存储` 的模块化单体方案。Web 层负责 UI 和 API，独立 worker 进程负责异步队列消费，所有模型请求统一经过适配层，所有产物围绕“项目 -> 任务 -> 资产/版本”落盘。
...
Run: `pnpm add @prisma/client bullmq ioredis zod bcryptjs jose`
...
Expected: Compose 文件能表达 `web + postgres + redis` 三服务关系。
...
### Task 4: 实现认证内核，包括密码哈希、会话 Cookie 和权限守卫
...
```ts
const token = await signSession({ userId: "u_1", role: "ADMIN" });
...
在 `src/lib/auth/session.ts` 中使用 `jose` 实现：
...
并输出 `signSession`、`verifySession`、`clearSessionCookie`。
